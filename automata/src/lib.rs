...
impl Automata {
		...
		for statement in &definition.statements {
			...
			match statement.match_kind {
				...
				StatementMatchKind::Sequence(ref sequence) => {
					self.process_sequence(sequence, &destination, &mut transition_table, definition.name);
				}
			}
		}
	...
	fn process_sequence(&mut self, sequence: &Vec<char>, destination: &Destination, source_state_transitions: &mut TransitionTable, source_name: InternedString) {
		...
		'sequence: for i in 0..sequence.len() {
			...
			match i {
				0 => {
					source_state_transitions.add_destination(
						input,
